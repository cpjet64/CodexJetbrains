...
- [x] [T9.1] Add log file rotation and levels (info/warn/error). (DiagnosticsService rotates log)
- [x] [T9.2] Capture CLI stderr lines to a Diagnostics tab. (DiagnosticsPanel streams stderr)
- [x] [T9.3] Add copy Diagnostics to clipboard action. (Copy button uses clipboard)
- [x] [T9.4] Detect stale process and auto-restart once. (ProcessHealthMonitor restarts stale CLI)
- [x] [T9.5] Backoff strategy on repeated failures. (ProtoSender exponential backoff restart)
- [x] [T9.6] Expose a Report Issue action with environment dump. (Action copies issue snapshot)
- [x] [T9.7] Add a health indicator in the status bar. (Status widget reflects ProcessHealth)
- [x] [T9.8] Track average tokens/sec for last turn. (TurnMetricsService updates tokens/sec)
- [x] [T9.9] Time each phase: stream, diff, apply, exec. (TurnMetricsService records durations)
- [x] [T9.10] Unit tests for restart/backoff logic. (ProcessHealthMonitorTest + ProtoSenderTest)
- [x] [T9.11] CI workflow for headless build with Gradle. (CI runs test + buildPlugin steps)
- [x] [T9.12] Document troubleshooting steps in README. (Troubleshooting section added)

## T10. Packaging and parity checks
- [ ] [T10.1] Add plugin icon and description polish. (no assets or copy updates)
- [/] [T10.2] Run buildPlugin; verify ZIP installs. (buildPlugin passes 2025-09-28; install open)
...
- [/] [T10.11] Post-tests for each earlier task must pass. (tests+buildPlugin pass; wrap-up todo)
